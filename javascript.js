...
$(function(){ 
	$('#main').mouseup(function(e) { 
		...
		if(inGame == 1) { 
			...
			if(lastNum == countNum) 
				endGame(1); //因为最后剩下的方块均为雷时应直接结束游戏，因此设置为剩余雷数和未被揭开的方块数相等的时候结束游戏 
		} 
		else if(inGame == 2) { 
...
function openNearBlock(x, y) { 
	...
	for(i = x - 1; i < x + 2; i ++) { 
		for(j = y - 1; j < y + 2; j ++) { 
			if(mineArray[i][j] != undefined) { 
				if($('#b' + i + '-' + j).hasClass('flag')) 
					flagNum ++; //统计方块周围的旗帜数和未揭开的方块数 
				if($('#b' + i + '-' + j).hasClass('hidden')) 
					hiddenNum ++; 
			} 
		} 
	} 
	if(flagNum == mineArray[x][y] && hiddenNum > flagNum) { //旗帜数和雷数相等且有未揭开方块且未插旗的方块时，则揭开它 
		for(i = x - 1; i < x + 2; i ++) { 
			for(j = y - 1; j < y + 2; j ++) { 
				if(mineArray[i][j] >= 0 && $('#b' + i + '-' + j).hasClass('hidden')) 
					openBlock(i, j); 
			} 
		} 
	} 
